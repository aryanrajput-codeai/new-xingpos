...
export function normalizeStatus(status: OrderStatus | string): string {
  ...
  if (s === "new order") return "new_order";
...
export class OrderLifecycleService {
  ...
  static getStandardNextStatus(order: Order): OrderStatus | null {
    ...
    if (norm === "new_order") {
      // For customer QR orders, accepting transitions New Order -> Confirmed
      // For POS orders, they are already Confirmed
      return order.orderSource === "POS" ? "Preparing" : "Confirmed";
    }
  ...
  static validateTransition(order: Order, targetStatus: OrderStatus, staff?: StaffMember): OrderTransitionValidation {
    ...
    // Forbid rejecting POS orders
    if (order.orderSource === "POS" && (targetNorm === "rejected")) {
      return { valid: false, reason: "Reject action is only available for customer QR Menu orders, not POS orders." };
    }

    // For QR Menu orders in New Order state
    if (currentNorm === "new_order") {
      if (targetNorm === "confirmed") {
        return { valid: true, requiredPermission: "order.confirm" };
      }
      if (targetNorm === "rejected" || targetNorm === "cancelled") {
  ...
  static getAvailableTransitions(order: Order, staff?: StaffMember): Array<{
    ...
    // For customer QR_MENU orders in New Order state: provide explicit Accept Order and Reject Order
    if (order.orderSource === "QR_MENU" && (currentNorm === "new_order" || order.orderStatus === "New Order")) {
      const confirmVal = this.validateTransition(order, "Confirmed", staff);
      transitions.push({
        targetStatus: "Confirmed",
        label: "Accept Order",
        description: "Accept and confirm QR menu order for kitchen processing",
        variant: "primary",
        requiresOverride: !!confirmVal.requiresManagerOverride,
        reason: confirmVal.reason
      });

      const rejectVal = this.validateTransition(order, "Cancelled", staff);
      transitions.push({
        targetStatus: "Cancelled",
        label: "Reject Order",
        description: "Reject customer QR menu order",
        variant: "danger",
        requiresOverride: !!rejectVal.requiresManagerOverride,
        reason: rejectVal.reason
      });
  ...
  static async transitionStatus(ctx: OrderTransitionContext): Promise<{ success: boolean; order: Order; message: string }> {
    ...
    if (order.orderType === "dine-in" && order.tableNumber) {
      ...
      if (targetTable) {
        if (targetNorm === "completed" || targetNorm === "cancelled" || targetNorm === "voided") {
          // If order is completed/cancelled and payment is settled (or order cancelled), free table
    ...
    if (targetNorm === "cancelled" || targetNorm === "voided") {
      // If order is cancelled or voided, reverse inventory consumption (Idempotent)
      InventoryConsumptionService.reverseOrderInventory(order.id, {
        actorName: ctx.user,
        reason: ctx.reason || `Order transition to '${ctx.targetStatus}'`
      }).catch(err => {
        console.warn("[Inventory Reversal Notice]:", err);
      });
    } else if (targetNorm !== "draft") {
      // If order enters active/confirmed/in-kitchen/completed state, consume inventory (Idempotent)
      InventoryConsumptionService.consumeOrderInventory(order, {
        actorName: ctx.user
      }).catch(err => {
  ...
  static getStageStatus(order: Order, stageKey: string): "completed" | "current" | "pending" | "failed" {
    ...
    if (currentNorm === "new_order") {
      // For QR orders pending confirmation, the first stage (confirmed) is pending/incoming
      return "pending";
    }
