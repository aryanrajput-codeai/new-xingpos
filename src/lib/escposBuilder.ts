...
export class ESCPOSBuilder {
  ...
  public itemRow(name: string, qty: string, price: string, total: string, width: "58mm" | "80mm" = "80mm"): this {
    return this.tableItemRow(name, qty, total || price, width);
  }

  /**
   * Formats item row with fixed-width monospaced columns:
   * 80mm: ITEM (24) | QTY (8) | AMOUNT (16) = 48 chars total
   * 58mm: ITEM (16) | QTY (4) | AMOUNT (12) = 32 chars total
   * Long item names wrap without shifting QTY or AMOUNT.
   * Amount is always right aligned. Quantity remains in its fixed column.
   */
  public tableItemRow(
    name: string,
    qty: string | number,
    ...
    const nameWidth = is80 ? 24 : 16;
    const qtyWidth = is80 ? 8 : 4;
    const amountWidth = is80 ? 16 : 12;

    const qtyStr = String(qty ?? 1).trim();
    const amountStr = String(amount ?? "").trim();

    // Word wrap item name to fit within nameWidth
    ...
    for (const word of words) {
      if (!current) {
        if (word.length <= nameWidth) {
          current = word;
        } else {
          let rem = word;
          while (rem.length > nameWidth) {
            nameLines.push(rem.slice(0, nameWidth));
            rem = rem.slice(nameWidth);
          }
          current = rem;
        }
      } else if (current.length + 1 + word.length <= nameWidth) {
        current += " " + word;
      } else {
    ...
    if (nameLines.length === 0) {
      nameLines.push(name.slice(0, nameWidth));
    }

    // Format QTY column (e.g. centered in 8 chars)
    const leftPad = Math.max(0, Math.floor((qtyWidth - qtyStr.length) / 2));
    const rightPad = Math.max(0, qtyWidth - qtyStr.length - leftPad);
    const formattedQty = " ".repeat(leftPad) + qtyStr + " ".repeat(rightPad);

    // Format AMOUNT column: strictly right aligned
    const formattedAmount = amountStr.padStart(amountWidth);

    // Line 1: Contains Name line 0, QTY, and AMOUNT
    const line0Name = nameLines[0].padEnd(nameWidth);
    this.writeText(line0Name + formattedQty + formattedAmount + "\n");

    // Subsequent lines: Wrapped name chunks with empty spaces for QTY and AMOUNT columns
    const emptyQty = " ".repeat(qtyWidth);
    const emptyAmount = " ".repeat(amountWidth);
    for (let i = 1; i < nameLines.length; i++) {
      const subName = nameLines[i].padEnd(nameWidth);
      this.writeText(subName + emptyQty + emptyAmount + "\n");
    }
