...
export function isAutoPrintEnabled(): boolean {
  ...
  return localStorage.getItem("ij_auto_print_enabled") !== "false" && localStorage.getItem("sr_auto_print_enabled") !== "false";
}

export function setAutoPrintEnabled(enabled: boolean): void {
  if (typeof window === "undefined") return;
  localStorage.setItem("ij_auto_print_enabled", String(enabled));
  localStorage.setItem("sr_auto_print_enabled", String(enabled));
...
export class PrintQueueManager {
  ...
  public static initAutoPrintListeners() {
    ...
    window.addEventListener("new_order", (e: any) => {
      ...
      if (!order) return;
      console.log("[PrintQueueManager Listener] New order registered (Status: NEW ORDER). No automatic printing triggered on order creation:", order.id);
      // STRICT REQUIREMENT: Creating an order must NEVER automatically print a bill or KOT.
    });

    // 2. Listen to "order_accepted" event (triggered ONLY when Admin manually clicks [Accept Order])
    window.addEventListener("order_accepted", (e: any) => {
      const order = e.detail as Order;
      if (!order) return;
      console.log("[PrintQueueManager Listener] Order manually accepted by Admin:", order.id);

      if (!isAutoPrintEnabled()) {
        console.log("[PrintQueueManager Listener] Automatic KOT printing is disabled in settings. Skipping automatic spooling.");
        return;
      }

      // STEP 5 (Option A): After Admin clicks Accept Order -> Print KOT only, if automatic KOT printing is enabled.
      console.log("[PrintQueueManager Listener] Spooling KOT for accepted order:", order.id);
      this.spoolKOT(order);
    });

    // 3. Listen to "order_updated_auto_print" event
    window.addEventListener("order_updated_auto_print", (e: any) => {
      const detail = e.detail;
      if (!detail || !detail.order) return;
      const { order, previousStatus } = detail;

      console.log(`[PrintQueueManager Listener] Order status updated: ${order.id}. Status: ${previousStatus} -> ${order.orderStatus}`);

      // If status changed to "Accepted" and auto-print is enabled, ensure KOT is spooled
      if (order.orderStatus === "Accepted" && previousStatus !== "Accepted" && isAutoPrintEnabled()) {
        console.log("[PrintQueueManager Listener] Order transitioned to Accepted. Spooling KOT.");
        this.spoolKOT(order);
      }
      // Note: Bills are NEVER automatically spooled on status transitions or payments. Bill printing is strictly on-demand.
    });
