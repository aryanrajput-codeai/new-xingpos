import { MenuItem, Review, KOT, KOTStatus, OrderItem, RestaurantTable, PrinterEmulatorLog, Category, PaymentRecord, PaymentStatus, PaymentMethod, SplitItemAssignment, SplitSettlement, Shift, ShiftStatus, CashAdjustment, CashAdjustmentType, ShiftFinancials, OrderStatus, OrderSource } from "../types";
...
export interface RestaurantSettings {
  ...
  // Professional Tax Invoice & Registration Fields
  estd?: string;
  ...
  // One-Click Silent Thermal Printing Configurations
  printingMode?: "silent" | "browser";
...
const generateMockOrders = (initialMenuItems: MenuItem[]): Order[] => {
  ...
  for (let i = 24; i >= 0; i--) {
    ...
    if (Math.random() > 0.6) {
      discountAmount = Math.round(subtotal * 0.2); // 20% coupon promo
      appliedCoupon = "XINGS20";
    }
...
const defaultSettings: RestaurantSettings = {
  name: RESTAURANT_BRANDING.name,
  estd: RESTAURANT_BRANDING.estd,
  establishedYear: "2024",
  legalName: RESTAURANT_BRANDING.legalName,
  tagline: RESTAURANT_BRANDING.tagline,
  contactNumber: RESTAURANT_BRANDING.contact.phone || "",
  email: RESTAURANT_BRANDING.contact.email,
  customerCare: RESTAURANT_BRANDING.contact.phone || "",
  address: RESTAURANT_BRANDING.contact.address || "Restaurant Address (Configure in Settings)",
  city: RESTAURANT_BRANDING.contact.city,
  state: "",
  country: "India",
  businessHours: RESTAURANT_BRANDING.contact.businessHours,
  deliveryCharges: 25,
  gstPercentage: 5,
  gstin: "",
  fssaiNumber: "",
  website: RESTAURANT_BRANDING.contact.website || "",
  customFooter: `${RESTAURANT_BRANDING.receipt.footerGreeting} • ${RESTAURANT_BRANDING.receipt.poweredByNote}`,
  ...
  googleMapsUrl: RESTAURANT_BRANDING.contact.googleMapsEmbedUrl || ""
};

// Initial logs
const defaultAuditLogs: AuditLog[] = [
  { id: "log-1", timestamp: new Date().toISOString(), user: "Admin (owner)", action: "System Initialized", details: `Local database initialized for ${RESTAURANT_BRANDING.name}. Ready for real orders.`, ipAddress: "127.0.0.1" }
];

// Self-executing migration to clean up all old simulated/mock transactions and prefilled data
if (typeof window !== "undefined") {
  const ERASE_VERSION = "v4_xings_kitchen";
...
export class LocalDB {
  ...
  static getMenuItems(): MenuItem[] {
    ...
    try {
      ...
      // Ensure any newly added default items (e.g. i12-i20 for demo recipes) are merged in
      ...
      if (updated) {
        localStorage.setItem("ij_menu_items", JSON.stringify(parsed));
      }
      return parsed;
    } catch (_) {
      localStorage.setItem("ij_menu_items", JSON.stringify(defaultMenuItems));
  ...
  static addOrder(order: Omit<Order, "id" | "createdAt">): Order {
    if (order.orderType === "dine-in" && order.tableNumber) {
      ...
      if (activeOrder) {
        ...
        if (idx !== -1) {
          ...
          // Add add-on KOT
          const isPosOrder = (order as any).orderSource === "POS" || activeOrder.orderSource === "POS";
          const freshKOT: KOT = {
            id: kotNumber,
            ...
            status: isPosOrder ? "Accepted" : "New Order",
            ...
            isAddOn: true,
            orderSource: isPosOrder ? "POS" : "QR_MENU"
          };
          ...
          // Phase 5: Automatic inventory consumption for newly added add-on items
          if (newlyAddedItemsWithTracking.length > 0) {
            import("./inventoryConsumptionService").then(({ InventoryConsumptionService }) => {
              InventoryConsumptionService.consumeOrderInventory({
                ...activeOrder,
                id: `${activeOrder.id}-addon-${addOnCount}`,
                items: newlyAddedItemsWithTracking
              }, {
                actorName: (order as any).billedBy || "POS Staff"
              }).catch(e => console.warn("[Add-on Consumption Notice]:", e?.message || e));
    ...
    const fullOrder: Order = {
      ...order,
      orderSource: order.orderSource || "QR_MENU",
      orderStatus: order.orderStatus || (isPos ? "Confirmed" : "New Order"),
      confirmedAt: isPos ? (order.confirmedAt || new Date().toISOString()) : order.confirmedAt,
    ...
    // Phase 5: Automatic POS Inventory Consumption
    const statusNorm = (fullOrder.orderStatus || "").trim().toLowerCase();
    if (statusNorm !== "draft" && statusNorm !== "cancelled" && statusNorm !== "voided") {
      import("./inventoryConsumptionService").then(({ InventoryConsumptionService }) => {
        InventoryConsumptionService.consumeOrderInventory(fullOrder, {
          actorName: (order as any).billedBy || "POS Staff"
        }).catch(consErr => {
          console.warn("[Inventory Consumption Notice]:", consErr?.message || consErr);
        });
      }).catch(e => console.warn(e));
    }
  ...
  static async fetchOrders(): Promise<Order[]> {
    ...
    try {
      const isRestricted = typeof sessionStorage !== "undefined" && sessionStorage.getItem("supabase_service_restricted") === "true";
      if (isRestricted) {
      ...
      if (error) {
        const isQuota = status === 402 || (error.message && (error.message.includes("exceed_egress_quota") || error.message.includes("restricted")));
        if (isQuota) {
          if (typeof sessionStorage !== "undefined") {
            sessionStorage.setItem("supabase_service_restricted", "true");
          }
          console.warn("[ORDER MANAGEMENT FETCH] Supabase egress quota exceeded (HTTP 402). Seamlessly reading from local ledger.");
        } else {
          console.warn("[ORDER MANAGEMENT FETCH] Supabase fetch warning:", error);
        }
        this.addAuditLog(
          "Supabase Notice",
          `HTTP ${status} - Remote order load note: ${error.message}. Loaded orders from local ledger.`,
          "System (Supabase)"
        );
      ...
      for (const item of (data || [])) {
        ...
        seenIds.add(item.id);
        const resolvedSource: OrderSource = item.order_source === "POS" ? "POS" :
          (item.order_source === "QR_MENU" ? "QR_MENU" : 
            (item.billed_by?.includes("POS") || item.payment_method?.includes("POS") ? "POS" : "QR_MENU"));
        mapped.push({
          id: item.id,
          ...
          orderStatus: item.order_status || (resolvedSource === "POS" ? "Confirmed" : "New Order"),
  ...
  static async apiMergeIntoExistingOrder(existingOrder: Order, newItems: any[], addedBy: string): Promise<Order> {
    ...
    // Add add-on KOT
    const isPosOrder = existingOrder.orderSource === "POS";
    const freshKOT: KOT = {
      id: kotNumber,
      ...
      isAddOn: true,
      orderSource: isPosOrder ? "POS" : (existingOrder.orderSource || "QR_MENU")
    };
  ...
  static async apiAddOrder(order: Omit<Order, "id" | "createdAt">): Promise<Order> {
    ...
    const orderSource: OrderSource = order.orderSource || "POS";
    const orderStatus: OrderStatus = order.orderStatus || (orderSource === "POS" ? "Confirmed" : "New Order");

    const fullOrder: Order = {
      ...order,
      orderSource,
      orderStatus,
      confirmedAt: orderSource === "POS" ? (order.confirmedAt || new Date().toISOString()) : order.confirmedAt,
    ...
    const payload: any = {
      ...
      order_source: fullOrder.orderSource,
    ...
    try {
      ...
      if (!isRestricted) {
        ...
        if (error) {
          console.warn(`[SUPABASE ORDER INSERT] Notice with status ${status}:`, error.message);
          
          const isQuota = status === 402 || 
            (error.message && (
              error.message.includes("exceed_egress_quota") ||
              error.message.includes("restricted") ||
              error.message.includes("spend caps") ||
          ...
          if (isQuota) {
            ...
            console.warn("[SUPABASE ORDER INSERT] Supabase egress quota exceeded (HTTP 402). Storing order in local resilient ledger.");
            this.addAuditLog(
              "Supabase Quota Notice",
              `Supabase egress quota limit reached (HTTP 402). Order ${newId} saved to resilient local ledger.`,
              "System"
            );
          } else {
            this.addAuditLog(
              "Supabase Sync Notice",
              `HTTP ${status} - Remote sync note: ${error.message} (${error.details || ""}). Saved to local ledger.`,
              "System"
            );
          }
        } else {
        ...
        debugLog("[SUPABASE ORDER INSERT] Local-first mode active (cloud quota limit reached). Order stored in local ledger.");
      }
    } catch (err: any) {
      console.warn("[SUPABASE ORDER INSERT] Network transport notice:", err);
      this.addAuditLog(
        "Offline Storage",
        `Order ${newId} stored in local resilient database.`,
        "System (Offline)"
      );
    }
    ...
    try {
      const isPos = fullOrder.orderSource === "POS";
      const initialKotStatus: KOTStatus = isPos ? "Accepted" : "New Order";
      const freshKOT: KOT = {
        id: kotNumber,
        orderId: fullOrder.id,
    ...
    // Phase 5: Automatic POS Inventory Consumption (Idempotent)
    const statusNorm = (fullOrder.orderStatus || "").trim().toLowerCase();
    if (statusNorm !== "draft" && statusNorm !== "cancelled" && statusNorm !== "voided") {
      import("./inventoryConsumptionService").then(({ InventoryConsumptionService }) => {
        InventoryConsumptionService.consumeOrderInventory(fullOrder, {
          actorName: (order as any).billedBy || "POS Cashier"
        }).catch(consErr => {
          console.warn("[Inventory Consumption Notice]:", consErr?.message || consErr);
        });
      }).catch(e => console.warn(e));
  ...
  static async apiUpdateOrderStatus(orderId: string, status: Order["orderStatus"], paymentStatus?: string, operator: string = "Staff"): Promise<Order> {
    ...
    if (idx !== -1) {
      ...
      for (const kot of kots) {
        if (kot.orderId === orderId) {
          if (sNorm === "preparing" && (kot.status === "New Order" || kot.status === "Accepted")) {
    ...
    if (orderCopy) {
      ...
      // Phase 5: Automatic POS Inventory Consumption / Reversal Integration
      const sNorm = (status || "").trim().toLowerCase();
      if (sNorm === "cancelled" || sNorm === "voided") {
        import("./inventoryConsumptionService").then(({ InventoryConsumptionService }) => {
          InventoryConsumptionService.reverseOrderInventory(orderId, {
            actorName: operator,
            reason: `Status updated to '${status}'`
          }).catch(err => {
            console.warn("[Inventory Reversal Notice]:", err);
          });
        }).catch(e => console.warn(e));
      } else if (sNorm !== "draft") {
        import("./inventoryConsumptionService").then(({ InventoryConsumptionService }) => {
          InventoryConsumptionService.consumeOrderInventory(orderCopy!, {
            actorName: operator
          }).catch(err => {
            console.warn("[Inventory Consumption Notice]:", err);
          });
        }).catch(e => console.warn(e));
      }
    }
  ...
  static async fetchMenuItems(): Promise<MenuItem[]> {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        console.log("[Supabase API] Menu table is empty in Supabase. Using default menu items.");
  ...
  static async apiSaveMenuItems(items: MenuItem[]): Promise<void> {
    ...
    try {
      ...
      const isQuota = (err.message && (err.message.includes("exceed_egress_quota") || err.message.includes("restricted") || err.message.includes("quota")));
      if (isQuota) {
        if (typeof sessionStorage !== "undefined") sessionStorage.setItem("supabase_service_restricted", "true");
        this.saveMenuItems(items);
        this.addAuditLog("Menu Catalog Saved (Offline)", `Catalog containing ${items.length} dishes saved to local resilient storage (cloud quota reached).`, "Admin (owner)");
        return;
      }
      throw err;
    }
  ...
  static async fetchSettings(): Promise<RestaurantSettings> {
    ...
    try {
      ...
      const mapped: RestaurantSettings = {
        name: item.name || RESTAURANT_BRANDING.name,
        contactNumber: item.contact_number || RESTAURANT_BRANDING.contact.phone,
    ...
    try {
      ...
      if (kotIdx !== -1) {
        ...
        if (status === "New Order") mappedOrderStatus = "New Order";
          ...
          mappedOrderStatus = linkedOrder?.orderSource === "POS" ? "Confirmed" : "Accepted";
        }
  ...
  static getPayments(): PaymentRecord[] {
    const stored = localStorage.getItem("sagar_pos_payments");
    if (!stored) {
      localStorage.setItem("sagar_pos_payments", JSON.stringify([]));
      return [];
    }
    try {
      const parsed = JSON.parse(stored);
  ...
  static savePayments(payments: PaymentRecord[]): void {
    ...
    localStorage.setItem("sagar_pos_payments", JSON.stringify(unique));
    window.dispatchEvent(new Event("storage"));
  ...
  static getShifts(): Shift[] {
    const stored = localStorage.getItem("sagar_pos_shifts");
    if (!stored) {
      localStorage.setItem("sagar_pos_shifts", JSON.stringify([]));
      return [];
    }
    try {
      const parsed = JSON.parse(stored);
  ...
  static saveShifts(shifts: Shift[]): void {
    ...
    localStorage.setItem("sagar_pos_shifts", JSON.stringify(unique));
    window.dispatchEvent(new Event("storage"));
  ...
  static getCashAdjustments(shiftId?: string): CashAdjustment[] {
    const stored = localStorage.getItem("sagar_pos_cash_adjustments");
  ...
  static saveCashAdjustments(adjustments: CashAdjustment[]): void {
    ...
    localStorage.setItem("sagar_pos_cash_adjustments", JSON.stringify(unique));
    window.dispatchEvent(new Event("storage"));
