...
import { formatKOTExactText } from "../lib/printerService";
...
export default function VirtualPrinterCenter() {
  ...
  // Generates thermal print receipt text using standardized KOT format
  const generateReceiptText = (kot: any) => {
    return formatKOTExactText(kot, { isReprint: Boolean(kot.isReprint || kot.printed) });
  };
  ...
  const triggerVirtualPrint = async (kot: any) => {
    if (isPrinting) {
      ...
      setQueue((prev) => [...prev, {
        kotId: kot.id,
        kotNumber: kot.id,
        restaurantId: "the-xings-kitchen-midc",
        printStatus: "Pending"
      }]);
      return;
    }
  ...
  const handleTestPrint = () => {
    const randomKot: KOT = {
      ...
      preparationTime: 12,
      items: [
        { menuItemId: "xk-1", name: "Steamed Veg Dim Sums", price: 180, quantity: 2 },
        { menuItemId: "xk-2", name: "Classic Hakka Noodles", price: 210, quantity: 1 },
        { menuItemId: "xk-3", name: "Veg Manchurian Dry", price: 220, quantity: 1 }
      ]
    };
                    ...
                    return (
                      <tr key={log.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="py-3 px-4 text-stone-800">
                          {BRAND_CONFIG.restaurantName} Central
                        </td>
