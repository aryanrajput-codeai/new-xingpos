...
export default function AdminLogin({ onLoginSuccess }: AdminLoginProps) {
  ...
  useEffect(() => {
    const savedEmail = localStorage.getItem("xk_admin_remember_email") || localStorage.getItem("ij_admin_remember_email");
    if (savedEmail) {
      setEmail(savedEmail);
    }
  }, []);
  ...
  const handleLoginSubmit = (e: React.FormEvent) => {
    ...
    // We authorize email: admin@xingskitchen.com or admin@idlijunction.com
    const isMasterEmail = email.toLowerCase() === "admin@xingskitchen.com" || email.toLowerCase() === "admin@idlijunction.com";
    ...
    if (isMasterEmail && isMasterPassword) {
      ...
      const payload = btoa(JSON.stringify({ sub: "xings_kitchen_admin_id", role: "Owner", email: email }));
      const header = btoa(JSON.stringify({ alg: "HS256", typ: "JWT" }));
      const mockSignature = "r9U_63r-9saV_77f_93n-c";
      const token = `${header}.${payload}.${mockSignature}`;

      if (rememberMe) {
        localStorage.setItem("xk_admin_remember_email", email);
      } else {
        localStorage.removeItem("xk_admin_remember_email");
      ...
      >
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              ...
              <input
                ...
                placeholder="admin@xingskitchen.com"
        ...
        <div className="mt-8 border-t border-stone-100 pt-4 text-center">
          <p className="text-[10px] sm:text-xs font-mono text-stone-400">
            ...
            <br />
            <span className="text-[#aa7c11]">admin@xingskitchen.com</span> / <span className="text-stone-600 font-bold">admin123</span>
          </p>
        </div>
      </motion.div>
              ...
              {recoverySuccess ? (
                <div className="py-6 text-center space-y-4">
                  ...
                  <p className="text-xs text-stone-600 max-w-sm mx-auto font-sans leading-relaxed">
                    A cryptographic security code and recovery instructions have been dispatched to your registered administrator email.
                  </p>
                </div>
              ) : (
