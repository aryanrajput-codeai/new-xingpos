...
import {
  ...
  FileSpreadsheet, Filter, Search, Sparkles, X, Check
} from "lucide-react";
...
interface ReportsDashboardProps {
  ...
  initialSubTab?: "overview" | "daily" | "monthly" | "yearly" | "items" | "categories" | "ledger";
  onSubTabChange?: (tab: "overview" | "daily" | "monthly" | "yearly" | "items" | "categories" | "ledger") => void;
}
...
export default function ReportsDashboard({
  ...
  // Sub-tabs: "overview" | "daily" | "monthly" | "yearly" | "items" | "categories" | "ledger"
  const [activeTab, setActiveTab] = useState<
    "overview" | "daily" | "monthly" | "yearly" | "items" | "categories" | "ledger"
  >(initialSubTab);
  ...
  const handleSubTabClick = (tab: "overview" | "daily" | "monthly" | "yearly" | "items" | "categories" | "ledger") => {
    setActiveTab(tab);
    onSubTabChange?.(tab);
  };
  ...
  const handleExportCSV = () => {
    const filename = `${BRAND_CONFIG.restaurantName.replace(/\s+/g, "_")}_Sales_${preset}_${getLocalDateKey(new Date())}.csv`;
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-stone-200/80 shadow-xs">
        <div className="space-y-1">
          ...
          <p className="text-xs text-stone-500 font-sans">
            Real-time culinary sales intelligence, item velocity, and revenue reconciliation for {BRAND_CONFIG.restaurantName}.
          </p>
        </div>
              ...
              <table className="w-full text-left text-xs font-sans border-collapse">
                ...
                <tbody className="divide-y divide-stone-100 text-stone-700">
                  {dailySales.filter(d => d.ordersCount > 0).length === 0 ? (
                    <tr>
                      <td colSpan={8} className="p-10 text-center text-stone-400 font-light">
              ...
              <table className="w-full text-left text-xs font-sans border-collapse">
                ...
                <tbody className="divide-y divide-stone-100 text-stone-700">
                  {filteredLedgerOrders.length === 0 ? (
                    <tr>
                      <td colSpan={9} className="p-12 text-center text-stone-400 font-light font-sans">
              ...
              <div className="p-4 sm:p-6 text-stone-800 font-sans space-y-6 bg-white" id="printable-sales-report">
                ...
                <div className="text-center border-b-2 border-stone-900 pb-4 space-y-1">
                  <h1 className="text-2xl font-serif font-bold text-stone-900 tracking-wider uppercase">
                    {BRAND_CONFIG.restaurantName.toUpperCase()}
                  </h1>
                  <p className="text-xs font-mono text-stone-500 uppercase tracking-widest">
                    {BRAND_CONFIG.cuisine} · POS Sales & Performance Audit
                  </p>
                ...
                <div className="grid grid-cols-3 gap-3 text-center border border-stone-300 p-4 rounded-xl">
                  <div>
                ...
                <div className="pt-8 border-t border-stone-300 grid grid-cols-2 gap-8 text-xs font-mono">
                  ...
                  <div className="text-right">
                    ...
                    <p className="mt-1 font-bold text-stone-800">{BRAND_CONFIG.restaurantName.toUpperCase()} Administration</p>
                  </div>
                </div>
              </div>
            </motion.div>
