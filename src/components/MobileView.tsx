...
export default function MobileView({
  ...
  const [showQrScannerMobile, setShowQrScannerMobile] = useState(false);
  const [showManualTablePickerMobile, setShowManualTablePickerMobile] = useState(false);
  ...
  // Pre-fill and synchronize table number from scanned table URL parameters with page refresh persistence
  useEffect(() => {
    const syncTableState = () => {
      try {
        ...
        if (tableVal) {
          ...
          setIsQrScannedMobile(active ? active.isFromQr : true);
        } else {
          setTableNumber("");
    ...
    const handleTableScannedEvent = (e: any) => {
      if (e.detail?.tableNumber) {
        ...
        setOrderType("dine-in");
        setIsQrScannedMobile(e.detail.isFromQr !== false);
  ...
  const filteredMenuList = useMemo(() => {
    return menuList.filter((item) => {
      ...
      if (selectedMobileCategory !== "all") {
        if ((item.category || "").toLowerCase() !== selectedMobileCategory.toLowerCase()) {
          return false;
        }
      }
  ...
  // Mobile categories specification matching live Supabase Chinese categories
  const mobileCategoryChips = useMemo(() => {
    ...
    // Add active categories dynamically
    list.forEach(c => {
      chips.push({ id: c.id, label: `${c.icon || "🥢"} ${c.name}`, emoji: c.icon || "🥢" });
    });
    
    // Also include any other category present in menuList not yet in list
    menuList.forEach(m => {
      if (m.category && !chips.some(chip => chip.id.toLowerCase() === m.category.toLowerCase())) {
        chips.push({ id: m.category, label: `🥢 ${m.category}`, emoji: "🥢" });
      }
    });
  ...
  return (
    ...
    >
      ...
      <div className="bg-stone-900 text-[10px] font-mono font-medium text-[#d4af37] border-b border-[#d4af37]/10 py-2 text-center tracking-wider uppercase flex items-center justify-between gap-1.5 px-3 sticky top-0 z-40">
        <div className="flex items-center gap-1.5 overflow-hidden">
          <Sparkles className="w-3.5 h-3.5 text-[#d4af37] animate-pulse shrink-0" />
          <span className="truncate">{BRAND_CONFIG.restaurantName}</span>
        </div>
        <div className="flex items-center gap-1.5 shrink-0">
          {tableNumber ? (
            <button
              onClick={() => setShowQrScannerMobile(true)}
              className="bg-[#d4af37]/20 border border-[#d4af37]/50 text-[#d4af37] px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1 cursor-pointer"
            >
            ...
            <button
              onClick={() => setShowQrScannerMobile(true)}
              className="bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30 px-2 py-0.5 rounded-full text-[9px] font-bold flex items-center gap-1 cursor-pointer"
            >
      ...
      {animateContainer(isLoading, activeTab, {
            ...
            >
              ...
              <div className="absolute bottom-6 left-6 right-6 space-y-3">
                <span className="text-[10px] font-mono tracking-[0.25em] text-[#d4af37] uppercase font-bold">
                  {BRAND_CONFIG.established} • NAGPUR
                </span>
                <h1 className="text-3xl font-serif font-extrabold text-white leading-tight tracking-wide uppercase">
                  {BRAND_CONFIG.restaurantName}
                  <span className="block text-transparent bg-clip-text bg-gradient-to-r from-[#d4af37] to-[#f3e5ab] text-xl font-medium mt-0.5 tracking-normal">
                    {BRAND_CONFIG.cuisine}
                  </span>
                </h1>
                  ...
                  {showOtpMobile ? (
                    ...
                    <>
                      ...
                      <div className="space-y-3">
                        ...
                        {orderType === "dine-in" && (
                          <div className="space-y-2.5 bg-stone-50 p-2.5 rounded-2xl border border-stone-200">
                            <div className="flex items-center justify-between">
                              <label className="text-[9px] text-stone-600 font-mono block font-bold uppercase tracking-wider flex items-center gap-1">
                              ...
                              {tableNumber && (
                                <button
                                  type="button"
                                  onClick={() => setShowQrScannerMobile(true)}
                                  className="text-[9.5px] text-[#aa7c11] font-mono font-bold flex items-center gap-1 cursor-pointer"
                                >
                                  <Camera className="w-2.5 h-2.5" />
                            ...
                            {!tableNumber ? (
                              <div className="space-y-2">
                                <div className="bg-amber-50 border border-amber-200 p-2 rounded-xl text-[10.5px] text-amber-900 flex items-start gap-1.5 font-sans">
                                  <Camera className="w-3.5 h-3.5 text-amber-600 shrink-0 mt-0.5" />
                                  <span>Scan your table QR code stand or pick your table number.</span>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                  <button
                                    type="button"
                                    onClick={() => setShowQrScannerMobile(true)}
                                    className="py-2 px-2.5 bg-[#d4af37] text-stone-950 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 shadow-sm cursor-pointer"
                                  >
                                  ...
                                  <button
                                    type="button"
                                    onClick={() => setShowManualTablePickerMobile(!showManualTablePickerMobile)}
                                    className="py-2 px-2.5 bg-white border border-stone-300 text-stone-700 font-semibold text-xs rounded-xl flex items-center justify-center gap-1 cursor-pointer"
                                  >
                                  ...
                                  <div className="pt-1">
                                    <select
                                      ...
                                      onChange={(e) => {
                                        ...
                                        setIsQrScannedMobile(false);
                                        setCheckoutErrorMobile(null);
                                        setShowManualTablePickerMobile(false);
                                      }}
                                      className="w-full bg-white text-stone-900 text-xs border border-stone-300 rounded-xl p-2 font-sans"
                                    >
                              ...
                              <div className="space-y-1.5">
                                <div className="bg-emerald-50 border border-emerald-200 p-2 rounded-xl text-[10.5px] text-emerald-900 flex items-center justify-between font-sans">
                                  <div className="flex items-center gap-1.5">
                                    <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                                    <span>
                                      <strong>Table #{tableNumber}</strong> {isQrScannedMobile ? "(QR Verified)" : "(Selected)"}
                                    </span>
                                  </div>
                                  <button
                                    type="button"
                                    ...
                                    className="text-[9px] text-stone-500 hover:text-red-600 font-mono underline cursor-pointer"
                                  >
                                    Clear
                                  </button>
                                </div>
                                <input type="hidden" required value={tableNumber} />
                              </div>
                            )}
                          </div>
                        )}
        ...
        contact: (
          <div className="px-4 py-6 animate-fade-in pb-12 space-y-6">
            ...
            {/* Restaurant Staff & Admin Portal Trigger */}
            <div className="pt-4 border-t border-stone-200/60 text-center pb-6">
              <button
                type="button"
                onClick={onAdminClick}
                className="text-[11px] font-mono text-stone-400 hover:text-stone-700 uppercase tracking-widest px-4 py-2 rounded-xl border border-dashed border-stone-300 hover:border-stone-400 transition-colors"
              >
                Restaurant Staff Portal / Admin Login
              </button>
            </div>
      ...
      {/* Mobile Live Camera QR Scanner Modal */}
      <QrScannerModal
        isOpen={showQrScannerMobile}
        onClose={() => setShowQrScannerMobile(false)}
        onScanSuccess={(table) => {
        ...
        subtitle="Point your camera at the table QR code stand"
      />
    </div>
  );
}
