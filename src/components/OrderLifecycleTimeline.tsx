...
import { OrderStatus, StaffMember, ManagerOverrideContext } from "../types";
...
import { OrderInventoryConsumption } from "../types/inventory";
...
export default function OrderLifecycleTimeline({
  ...
  const [consumption, setConsumption] = useState<OrderInventoryConsumption | null>(null);
  ...
  useEffect(() => {
    ...
    const loadConsumption = async () => {
      try {
        const record = await InventoryConsumptionService.getOrderConsumption(order.id);
        if (isMounted) {
    ...
    const handleConsumptionUpdate = () => loadConsumption();
    window.addEventListener("order_inventory_consumed", handleConsumptionUpdate);
    window.addEventListener("inventory_transactions_updated", handleConsumptionUpdate);

    return () => {
      isMounted = false;
      window.removeEventListener("order_inventory_consumed", handleConsumptionUpdate);
      window.removeEventListener("inventory_transactions_updated", handleConsumptionUpdate);
    };
  }, [order.id]);
  ...
  const handleActionClick = (targetStatus: OrderStatus) => {
    ...
    if (targetStatus === "Cancelled" || targetStatus === "Voided") {
      setPendingTargetStatus(targetStatus);
      setCancelPromptOpen(true);
      return;
    }

    if (targetStatus === "Out for Delivery" && !order.deliveryPartner) {
      setPendingTargetStatus(targetStatus);
      setDeliveryPromptOpen(true);
      return;
    }

    if (validation.requiresManagerOverride) {
      setPendingTargetStatus(targetStatus);
      setOverrideContext({
        actionType: (targetStatus as any) === "Voided" ? "void_order" : "rollback_status",
        actionLabel: `Advance / Revert Order to ${targetStatus}`,
        requiredPermission: validation.requiredPermission || "order.rollback",
        targetId: order.id,
        targetDetails: validation.reason
      });
  ...
  const handleOverrideAuthorized = (manager: StaffMember, reason: string) => {
    setShowOverrideModal(false);
    if (pendingTargetStatus) {
      executeTransition(pendingTargetStatus, reason, manager.name);
    }
  };
  ...
  return (
    <div className="space-y-4">
          ...
          >
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                ...
                onClick={() => {
                  if (pendingTargetStatus) {
                    const validation = OrderLifecycleService.validateTransition(order, pendingTargetStatus, staff);
                    if (validation.requiresManagerOverride) {
                      setOverrideContext({
                        actionType: pendingTargetStatus === "Voided" ? "void_order" : "cancel_order",
                        actionLabel: `${pendingTargetStatus} Order #${order.id}`,
                        requiredPermission: validation.requiredPermission || "order.cancel",
                        targetId: order.id,
                        targetDetails: cancelReason.trim()
                      });
                      setShowOverrideModal(true);
                    } else {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Ramesh / Delivery Partner"
      ...
      {/* Phase 5: Inventory & Recipe Consumption Card */}
      {consumption && (
        <div className="border border-stone-200/80 rounded-2xl bg-white overflow-hidden shadow-2xs">
          <div className="p-4 bg-stone-50/70 border-b border-stone-200/60 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded-full border ${
                    ...
                    {consumption.status === 'CONSUMED'
                      ? 'STOCK CONSUMED'
                      : consumption.status === 'REVERSED'
                      ? 'STOCK RESTORED (REVERSED)'
                      : 'NO RECIPES CONFIGURED'}
                  </span>
                </div>
                <div className="text-[11px] font-mono text-stone-500 mt-0.5 flex items-center gap-2">
                  <span>COGS: <strong className="text-stone-900">₹{consumption.totalCost.toFixed(2)}</strong></span>
                  <span>•</span>
                  <span>{consumption.totalItemsConsumed} item(s) mapped</span>
                  <span>•</span>
                  <span>{new Date(consumption.consumedAt).toLocaleTimeString()}</span>
                </div>
              </div>
            </div>
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-lg text-xs font-mono font-medium text-stone-600 hover:text-stone-900 hover:bg-stone-200/60 transition-colors flex items-center gap-1 cursor-pointer"
            >
              <span>{showConsumptionDetails ? "Hide" : "Details"}</span>
              {showConsumptionDetails ? <ChevronUp className="w-3.5 h-3.5" /> : <ChevronDown className="w-3.5 h-3.5" />}
            </button>
          </div>

          {/* Missing Recipes Warning */}
          {consumption.missingRecipes && consumption.missingRecipes.length > 0 && (
            <div className="p-3 bg-amber-50/60 border-b border-amber-200/50 flex items-start gap-2.5 text-xs text-amber-900 font-mono">
              <AlertCircle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
              <div>
                <span className="font-bold">Missing Recipes Notice: </span>
                <span>The following items have no active recipe mapped, so stock deduction was skipped: </span>
                <span className="font-semibold text-amber-950">
                  {consumption.missingRecipes.map((m) => `${m.quantity}x ${m.menuItemName}`).join(", ")}
                </span>
              </div>
            </div>
          )}
          ...
          {consumption.status === 'REVERSED' && (
            <div className="p-3 bg-rose-50/60 border-b border-rose-200/50 flex items-start gap-2.5 text-xs text-rose-900 font-mono">
              <RotateCcw className="w-4 h-4 text-rose-600 shrink-0 mt-0.5" />
              <div>
                <span className="font-bold">Inventory Returned to Stock: </span>
                <span>All consumed ingredients were restored upon cancellation. </span>
                {consumption.reversalReason && (
                  <span className="text-rose-950">Reason: {consumption.reversalReason}</span>
                )}
                {consumption.reversedAt && (
                  <span className="text-stone-500 ml-1">({new Date(consumption.reversedAt).toLocaleTimeString()})</span>
                )}
              </div>
            </div>
          )}
          ...
          {showConsumptionDetails && consumption.items && consumption.items.length > 0 && (
            <div className="p-4 space-y-4">
              {consumption.items.map((item, iIdx) => (
                <div key={iIdx} className="rounded-xl border border-stone-200/70 p-3 bg-stone-50/30">
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] text-stone-500 font-mono bg-stone-100 px-1.5 py-0.5 rounded">
                        Recipe #{item.recipeId.substring(0, 8)}
                      </span>
                    </div>
                    <span className="text-xs font-mono font-bold text-stone-700">
                      ₹{item.ingredients.reduce((s, ing) => s + ing.lineCost, 0).toFixed(2)}
                    </span>
                  </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-left text-xs font-mono">
                      <thead>
                        <tr className="text-[10px] text-stone-400 uppercase border-b border-stone-200/60">
                          <th className="pb-1.5 font-medium">Ingredient</th>
                          <th className="pb-1.5 font-medium">Req / Serving</th>
                          <th className="pb-1.5 font-medium">Waste %</th>
                          <th className="pb-1.5 font-medium">Total Deducted</th>
                          <th className="pb-1.5 font-medium">Cost / Unit</th>
                          <th className="pb-1.5 font-medium text-right">Line Cost</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-stone-100">
                        {item.ingredients.map((ing, ingIdx) => (
                          <tr key={ingIdx} className="text-stone-700">
                            <td className="py-1.5 font-semibold text-stone-900">{ing.ingredientName}</td>
                            <td className="py-1.5 text-stone-600">
                              {ing.requiredQuantity} {ing.requiredUnit}
                            </td>
                            <td className="py-1.5 text-stone-500">{ing.wastePercentage}%</td>
                            <td className="py-1.5 font-bold text-stone-900">
                              -{ing.normalizedQuantity} {ing.ingredientUnit}
                            </td>
                            <td className="py-1.5 text-stone-600">₹{ing.unitCost.toFixed(2)}</td>
                            <td className="py-1.5 font-bold text-stone-900 text-right">
