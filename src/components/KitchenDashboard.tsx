...
import { PhysicalThermalPrinter, formatKOTExactText } from "../lib/printerService";
...
export default function KitchenDashboard() {
  ...
  // Auto processing effects: detect unprinted KOTs (Only for Accepted or Preparing orders!)
  useEffect(() => {
    if (autoPrint && printerStatus === "connected" && kots.length > 0) {
      const unprinted = kots.filter(k => !k.printed && k.status !== "Cancelled" && k.status !== "New Order");
  ...
  return (
    <div className="space-y-6">
      ...
      {printKOT && (
        <div className="hidden print:block print:p-4 bg-white text-black font-mono text-xs max-w-[80mm] mx-auto">
          <pre className="font-mono text-xs font-bold whitespace-pre text-black leading-tight text-left">
            {formatKOTExactText(printKOT, { isReprint: Boolean(printKOT.printed) })}
          </pre>
        </div>
      )}
              ...
              {activePrintingKot ? (
                <div className="absolute top-11 left-1/2 -translate-x-1/2 w-[85%] bg-white text-stone-900 p-3 shadow-2xl rounded-b-md border-t border-dashed border-stone-300 origin-top text-left font-mono text-[8px] leading-tight select-none z-10 animate-[slideDown_1.5s_ease-out_forwards]">
                  ...
                  <div className="text-center font-bold pb-1 border-b border-dashed border-stone-300 mb-1">
                    <p className="text-[9px]">** {BRAND_CONFIG.restaurantName.toUpperCase()} **</p>
            ...
            {activePrinterTab === "emulator" && (
              <div className="pt-3 space-y-3">
                ...
                <div className="bg-stone-950 rounded-xl p-3 border border-stone-850 h-[160px] overflow-y-auto text-[10px] font-mono whitespace-pre text-stone-300 select-all scrollbar-thin">
                  {kots.length > 0 ? (
                    formatKOTExactText(kots[0], { isReprint: Boolean(kots[0].printed) })
                  ) : (
