...
import { parseCurrentRoute, navigateTo } from "./lib/router";
import { detectTableFromCurrentUrl } from "./lib/qrHelper";
...
export default function App() {
  ...
  // Dynamic menu items read directly from live Supabase MenuService
  const [menuList, setMenuList] = useState<MenuItem[]>(() => MenuService.getLiveMenuItemsSync());
  ...
  useEffect(() => {
    ...
    // Detect if customer arrived via table QR scan URL parameter (?table=...)
    detectTableFromCurrentUrl();

    const handleSync = () => {
      setMenuList(MenuService.getLiveMenuItemsSync());
    };
    window.addEventListener("storage", handleSync);
    // Custom internal dispatch sync
    window.addEventListener("menu_updated", handleSync);

    // Dynamic backend catalog sync on mount from LIVE Supabase
    const loadRealData = async () => {
      try {
        const items = await MenuService.getMenuItems();
        setMenuList(items);
      } catch (err) {
        console.warn("[App] Initial menu fetch notice:", err);
      }
    };
    loadRealData();
  ...
  const handleAdminLoginSuccess = (token: string, rememberMe: boolean) => {
    ...
    // Admin login clears dining table session
    try {
    ...
    if (route.view !== "admin" || route.rawPath === "/login" || route.rawPath === "/admin") {
...
function NoItemsState({ searchQuery }: NoItemsStateProps) {
  return (
    <div className="py-24 text-center max-w-md mx-auto" id="no-items-state">
      <div className="w-16 h-16 rounded-full bg-white border border-stone-150 flex items-center justify-center text-[#d4af37] mx-auto mb-4 shadow-sm">
        <Utensils className="w-7 h-7" />
      </div>
      <h3 className="text-lg font-serif font-bold text-stone-900 uppercase tracking-wider">
        {searchQuery ? "No Matching Dishes Found" : "Menu Is Currently Empty"}
      </h3>
      <p className="text-sm text-stone-500 font-sans font-light mt-2 leading-relaxed">
        {searchQuery ? (
          ...
          "No dishes in the menu catalog yet. Insert new menu items into your Supabase database or add them via the Admin Dashboard."
        )}
      </p>
    </div>
  );
